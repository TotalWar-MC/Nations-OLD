...
public class Permission {
	...
	public void change(PermissionChange permChange) {
		change(permChange.getChangeAction(), permChange.getChangeValue(), permChange.getArgs());
	}
	
	public void change(PermissionChange.Action permChange, boolean toValue, Object... args) {
		// Sorted by most common to least common
		if (permChange == PermissionChange.Action.SINGLE_PERM && args.length == 2) {
			perms[((PermLevel) args[0]).getIndex()][((ActionType) args[1]).getIndex()] = toValue;
		}
		else if (permChange == PermissionChange.Action.PERM_LEVEL && args.length == 1) {
			Arrays.fill(perms[((PermLevel) args[0]).getIndex()], toValue);
		}
		else if (permChange == PermissionChange.Action.ACTION_TYPE && args.length == 1) {
			for (PermLevel permLevel : PermLevel.values) {
				perms[permLevel.getIndex()][((ActionType) args[0]).getIndex()] = toValue;
			}
		}
		else if (permChange == PermissionChange.Action.ALL_PERMS) {
			setAllNonEnvironmental(toValue);
		}
		else if (permChange == PermissionChange.Action.RESET && args.length == 1) {
	...
	public void loadDefault(TownBlockOwner owner) {
		...
		for (PermLevel permLevel : PermLevel.values) {
			for (ActionType actionType : ActionType.values) {
				perms[permLevel.getIndex()][actionType.getIndex()] = Settings.getDefaultPermission(owner, permLevel, actionType);
			}
		}

		if (owner instanceof Town) {
			pvp = Settings.getPermFlag_Town_Default_PVP();
			fire = Settings.getPermFlag_Town_Default_FIRE();
			explosion = Settings.getPermFlag_Town_Default_Explosion();
			mobs = Settings.getPermFlag_Town_Default_Mobs();
		} else {
