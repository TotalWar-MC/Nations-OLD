 ...
 * 
 */
public class GroupManagerSource extends NationsPermissionSource {

	public GroupManagerSource(Main plugin, Plugin test) {
	...
	public String getPrefixSuffix(Resident resident, String node) {
		...
		if (!group.equals(user))
			user = group + user;
		user = Settings.parseSingleLineString(user);
		...
		public void onGMUserEvent(GMUserEvent event) {
			...
			try {
				if (PermissionEventEnums.GMUser_Action.valueOf(event.getAction().name()) != null) {
					...
					try {
						resident = NationsUniverse.getInstance().getDataSource().getResident(event.getUserName());
