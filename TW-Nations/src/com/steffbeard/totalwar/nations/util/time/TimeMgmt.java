package com.steffbeard.totalwar.nations.util.time;
...
public class TimeMgmt {
	...
	public static String formatCountdownTime(long l) {
		...
		if (l >= 3600) {
			...
			out = h + Settings.getLangString("msg_hours");
			l -= h * 3600;
		}
		if (l >= 60) {
			int m = (int) Math.floor(l / 60);
			out += (out.length() > 0 ? ", " : "") + m + Settings.getLangString("msg_minutes");
			l -= m * 60;
		}
		if (out.length() == 0 || l > 0)
			out += (out.length() > 0 ? ", " : "") + l + Settings.getLangString("msg_seconds");
		return out;
	}
	...
	public static String getFormattedTimeValue(double timeMillis) {
        ...
        if(timeMillis> 0) {
            ...
            return "0" + Settings.getLangString("msg_seconds");
        }
    }
}
