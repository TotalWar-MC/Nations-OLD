...
public class CombatUtil {
	 ...
	 */
	public static boolean preventDamageCall(Main plugin, Entity attacker, Entity defender) {
		...
		if (attackingPlayer != null) {
			...
			if ((defendingPlayer != null) || ((defenderTB != null) && ((defendingEntity instanceof Wolf) && ((Wolf) defendingEntity).isTamed() && !((Wolf) defendingEntity).getOwner().equals((AnimalTamer) attackingEntity)))) {
				...
				if (preventFriendlyFire(attackingPlayer, defendingPlayer)
					...
					|| (Settings.getWarSiegeEnabled()
						&& Settings.getWarSiegePostSpawnDamageImmunityEnabled()
				 ...
				 */
				if (defenderTB != null) {
					if(defenderTB.getType() == TownBlockType.FARM && Settings.getFarmAnimals().contains(defendingEntity.getType().toString())) {
						if (PlayerCacheUtil.getCachePermission(attackingPlayer, attackingPlayer.getLocation(), Material.WHEAT, ActionType.DESTROY))
							return false;
					}
					List<Class<?>> prots = EntityTypeUtil.parseLivingEntityClassNames(Settings.getEntityTypes(), "TownMobPVM:");
				...
				if (block != null) {
					...
					boolean bDestroy = PlayerCacheUtil.getCachePermission(attackingPlayer, defendingEntity.getLocation(), block, Permission.ActionType.DESTROY);
					...
					if (!bDestroy) {
						...
						if (cache.hasBlockErrMsg())
							Messages.sendErrorMsg(attackingPlayer, cache.getBlockErrMsg());
	 ...
	 */
	public static boolean preventPvP(NationsWorld world, TownBlock townBlock) {
		...
		if (townBlock != null) {
			try {
				...
				if (townBlock.isHomeBlock() && world.isForcePVP() && Settings.isForcePvpNotAffectingHomeblocks())
					return true;
	 ...
	 */
	public static boolean isWorldPvP(NationsWorld world) {
	...
	public static boolean preventFriendlyFire(Player attacker, Player defender) {
		...
		if ((attacker != null) && (defender != null))
			if (!Settings.getFriendlyFire() && CombatUtil.isAlly(attacker.getName(), defender.getName())) {
				try {
					...
					if (!townBlock.getType().equals(TownBlockType.ARENA))
						attacker.sendMessage(Settings.getLangString("msg_err_friendly_fire_disable"));
						return true;
	...
	public static boolean isAlly(String attackingResident, String defendingResident) {
		...
		try {
			Resident residentA = NationsUniverse.getDataSource().getResident(attackingResident);
			Resident residentB = NationsUniverse.getDataSource().getResident(defendingResident);
	...
	public static boolean canAttackEnemy(String a, String b) {
		...
		try {
			Resident residentA = NationsUniverse.getDataSource().getResident(a);
			Resident residentB = NationsUniverse.getDataSource().getResident(b);
			if (residentA.getTown() == residentB.getTown())
				return false;
			if (residentA.getTown().getNation() == residentB.getTown().getNation())
				return false;
	...
	public boolean isEnemyTownBlock(Player player, WorldCoord worldCoord) {
		...
		try {
			return CombatUtil.isEnemy(NationsUniverse.getInstance().getDataSource().getResident(player.getName()).getTown(), worldCoord.getTownBlock().getTown());
		} catch (NotRegisteredException e) {
			return false;
		}
	}
