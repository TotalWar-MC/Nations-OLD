...
public class AreaSelectionUtil {
	...
	public static List<WorldCoord> selectWorldCoordArea(TownBlockOwner owner, WorldCoord pos, String[] args) throws NationsException {

		List<WorldCoord> out = new ArrayList<>();

		if (args.length == 0) {
			// claim with no sub command entered so attempt selection of one plot
			if (pos.getNationsWorld().isClaimable())
				out.add(pos);
			else
				throw new NationsException(Settings.getLangString("msg_not_claimable"));
		} else {
			if (args.length > 1) {
				if (args[0].equalsIgnoreCase("rect")) {
					...
					throw new NationsException(String.format(Settings.getLangString("msg_err_invalid_property"), StringMgmt.join(args, " ")));
				}
				...
				if (!tb.isOutpost() && tb.hasTown()) { // isOutpost(), only for mysql however, if we include this we can skip the outposts on flatfile so less laggy!
					...
					if (NationsUniverse.getInstance().isTownBlockLocContainedInTownOutposts(town.getAllOutpostSpawns(), tb)) {
						tb.setOutpost(true);
						out.add(pos);
					} else {
						throw new NationsException(Settings.getLangString("msg_err_unclaim_not_outpost"));
				...
				try {
					...
					throw new NationsException(String.format(Settings.getLangString("msg_err_invalid_property"), args[0]));
				}
			}
		}
	...
	public static List<WorldCoord> selectWorldCoordAreaRect(TownBlockOwner owner, WorldCoord pos, String[] args) throws NationsException {
		...
		if (pos.getNationsWorld().isClaimable()) {
			if (args.length > 0) {
				...
				if (owner instanceof Town) {
					...
					available = Settings.getMaxTownBlocks(town) - town.getTownBlocks().size();
				} else if (owner instanceof Resident) {
					available = Settings.getMaxResidentPlots((Resident) owner);
				}
				...
				if (args[0].equalsIgnoreCase("auto")) {
					...
					try {
						...
						throw new NationsException(Settings.getLangString("msg_err_invalid_radius"));
					}
				}
				if (r > Settings.getMaxClaimRadiusValue() && Settings.getMaxClaimRadiusValue() > 0) {
					throw new NationsException(String.format(Settings.getLangString("msg_err_invalid_radius_number"),Settings.getMaxClaimRadiusValue()));
				}
	...
	public static List<WorldCoord> selectWorldCoordAreaCircle(TownBlockOwner owner, WorldCoord pos, String[] args) throws NationsException {
	...
	public static List<WorldCoord> filterInvalidProximityTownBlocks(List<WorldCoord> selection, Town town) {
		...
		for (WorldCoord worldCoord : selection)
			try {
				if (worldCoord.getNationsWorld().getMinDistanceFromOtherTownsPlots(worldCoord, town) >= Settings.getMinDistanceFromTownPlotblocks()) {
					out.add(worldCoord);
				} else {
					Messages.sendDebugMsg("AreaSelectionUtil:filterInvalidProximity - Coord: " + worldCoord + " too close to another town." );					
				}
	...
	public static boolean isOnEdgeOfOwnership(TownBlockOwner owner, WorldCoord worldCoord) {
		...
		for (int i = 0; i < 4; i++)
			try {
				TownBlock edgeTownBlock = worldCoord.getNationsWorld().getTownBlock(new Coord(worldCoord.getX() + offset[i][0], worldCoord.getZ() + offset[i][1]));
