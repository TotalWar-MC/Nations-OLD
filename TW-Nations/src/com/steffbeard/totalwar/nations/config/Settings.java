...
public class Settings {
	...
	private static final SortedMap<Integer, Map<Settings.TownLevel, Object>> configTownLevel = Collections.synchronizedSortedMap(new TreeMap<Integer, Map<Settings.TownLevel, Object>>(Collections.reverseOrder()));
	private static final SortedMap<Integer, Map<Settings.NationLevel, Object>> configNationLevel = Collections.synchronizedSortedMap(new TreeMap<Integer, Map<Settings.NationLevel, Object>>(Collections.reverseOrder()));
	...
	public static void newTownLevel(int numResidents, String namePrefix, String namePostfix, String mayorPrefix, String mayorPostfix, int townBlockLimit, double townUpkeepMultiplier, int townOutpostLimit, int townBlockBuyBonusLimit) {
		...
		ConcurrentHashMap<Settings.TownLevel, Object> m = new ConcurrentHashMap<Settings.TownLevel, Object>();
		m.put(Settings.TownLevel.NAME_PREFIX, namePrefix);
		m.put(Settings.TownLevel.NAME_POSTFIX, namePostfix);
		m.put(Settings.TownLevel.MAYOR_PREFIX, mayorPrefix);
		m.put(Settings.TownLevel.MAYOR_POSTFIX, mayorPostfix);
		m.put(Settings.TownLevel.TOWN_BLOCK_LIMIT, townBlockLimit);
		m.put(Settings.TownLevel.UPKEEP_MULTIPLIER, townUpkeepMultiplier);
		m.put(Settings.TownLevel.OUTPOST_LIMIT, townOutpostLimit);
		m.put(Settings.TownLevel.TOWN_BLOCK_BUY_BONUS_LIMIT, townBlockBuyBonusLimit);
	...
	public static void newNationLevel(int numResidents, String namePrefix, String namePostfix, String capitalPrefix, String capitalPostfix, String kingPrefix, String kingPostfix, int townBlockLimitBonus, double nationUpkeepMultiplier, double nationTownUpkeepMultiplier, int nationZonesSize, int nationBonusOutpostLimit) {
		...
		ConcurrentHashMap<Settings.NationLevel, Object> m = new ConcurrentHashMap<Settings.NationLevel, Object>();
		m.put(Settings.NationLevel.NAME_PREFIX, namePrefix);
		m.put(Settings.NationLevel.NAME_POSTFIX, namePostfix);
		m.put(Settings.NationLevel.CAPITAL_PREFIX, capitalPrefix);
		m.put(Settings.NationLevel.CAPITAL_POSTFIX, capitalPostfix);
		m.put(Settings.NationLevel.KING_PREFIX, kingPrefix);
		m.put(Settings.NationLevel.KING_POSTFIX, kingPostfix);
		m.put(Settings.NationLevel.TOWN_BLOCK_LIMIT_BONUS, townBlockLimitBonus);
		m.put(Settings.NationLevel.UPKEEP_MULTIPLIER, nationUpkeepMultiplier);
		m.put(Settings.NationLevel.NATION_TOWN_UPKEEP_MULTIPLIER, nationTownUpkeepMultiplier);
		m.put(Settings.NationLevel.NATIONZONES_SIZE, nationZonesSize);
		m.put(Settings.NationLevel.NATION_BONUS_OUTPOST_LIMIT, nationBonusOutpostLimit);
	...
	public static Map<Settings.TownLevel, Object> getTownLevel(int numResidents) {

		return configTownLevel.get(numResidents);
	}

	public static Map<Settings.NationLevel, Object> getNationLevel(int numResidents) {

		return configNationLevel.get(numResidents);
	}

	public static Map<Settings.TownLevel, Object> getTownLevel(Town town) {

		return getTownLevel(calcTownLevel(town));
	}

	public static Map<Settings.NationLevel, Object> getNationLevel(Nation nation) {
...
//		NationsWarConfig.setEditableMaterialsInWarZone(getAllowedMaterials(ConfigNodes.WAR_WARZONE_EDITABLE_MATERIALS));
	...
	public static String getKingPrefix(Resident resident) {
		...
		try {
			return (String) getNationLevel(resident.getTown().getNation()).get(Settings.NationLevel.KING_PREFIX);
		} catch (NotRegisteredException e) {
	...
	public static String getMayorPrefix(Resident resident) {
		...
		try {
			return (String) getTownLevel(resident.getTown()).get(Settings.TownLevel.MAYOR_PREFIX);
		} catch (NotRegisteredException e) {
	...
	public static String getCapitalPostfix(Town town) {
		...
		try {
			return ChatColor.translateAlternateColorCodes('&',(String) getNationLevel(town.getNation()).get(Settings.NationLevel.CAPITAL_POSTFIX));
		} catch (NotRegisteredException e) {
	...
	public static String getTownPostfix(Town town) {
		...
		try {
			return ChatColor.translateAlternateColorCodes('&',(String) getTownLevel(town).get(Settings.TownLevel.NAME_POSTFIX));
		} catch (Exception e) {
	...
	public static String getNationPostfix(Nation nation) {
		...
		try {
			return ChatColor.translateAlternateColorCodes('&',(String) getNationLevel(nation).get(Settings.NationLevel.NAME_POSTFIX));
		} catch (Exception e) {
	...
	public static String getNationPrefix(Nation nation) {
		...
		try {
			return ChatColor.translateAlternateColorCodes('&',(String) getNationLevel(nation).get(Settings.NationLevel.NAME_PREFIX));
		} catch (Exception e) {
	...
	public static String getTownPrefix(Town town) {
		...
		try {
			return ChatColor.translateAlternateColorCodes('&',(String) getTownLevel(town).get(Settings.TownLevel.NAME_PREFIX));
		} catch (Exception e) {
	...
	public static String getCapitalPrefix(Town town) {
		...
		try {
			return ChatColor.translateAlternateColorCodes('&',(String) getNationLevel(town.getNation()).get(Settings.NationLevel.CAPITAL_PREFIX));
		} catch (NotRegisteredException e) {
	...
	public static String getKingPostfix(Resident resident) {
		...
		try {
			return (String) getNationLevel(resident.getTown().getNation()).get(Settings.NationLevel.KING_POSTFIX);
		} catch (NotRegisteredException e) {
	...
	public static String getMayorPostfix(Resident resident) {
		...
		try {
			return (String) getTownLevel(resident.getTown()).get(Settings.TownLevel.MAYOR_POSTFIX);
		} catch (NotRegisteredException e) {
	...
	public static int getMaxTownBlocks(Town town) {
		...
		if (ratio == 0) {
			n += (Integer) getTownLevel(town).get(Settings.TownLevel.TOWN_BLOCK_LIMIT);
		...
		if (town.hasNation())
			try {
				n += (Integer) getNationLevel(town.getNation()).get(Settings.NationLevel.TOWN_BLOCK_LIMIT_BONUS);
			} catch (NotRegisteredException e) {
			}
	...
	public static int getMaxOutposts(Town town) {
		...
		int townOutposts = (Integer) getTownLevel(town).get(Settings.TownLevel.OUTPOST_LIMIT);
		int nationOutposts = 0;
		if (town.hasNation())
			try {
				nationOutposts = (Integer) getNationLevel(town.getNation()).get(Settings.NationLevel.NATION_BONUS_OUTPOST_LIMIT);
			} catch (NotRegisteredException e) {
			}
	...
	public static int getMaxBonusBlocks(Town town) {
		...
		return (Integer) getTownLevel(town).get(Settings.TownLevel.TOWN_BLOCK_BUY_BONUS_LIMIT);
	}

	public static int getNationBonusBlocks(Nation nation) {

		return (Integer) getNationLevel(nation).get(Settings.NationLevel.TOWN_BLOCK_LIMIT_BONUS);
	}
    ...
    public static double getTownUpkeepCost(Town town) {
        ...
        if (town != null) {
            if (isUpkeepByPlot()) {
                ...
                multiplier = Double.valueOf(getTownLevel(town).get(Settings.TownLevel.UPKEEP_MULTIPLIER).toString());
            }
        } else
            multiplier = 1.0;
        ...
        if (town.hasNation()) {
            ...
            try {
                nationMultiplier = Double.valueOf(getNationLevel(town.getNation()).get(Settings.NationLevel.NATION_TOWN_UPKEEP_MULTIPLIER).toString());
            } catch (NumberFormatException e) {
                e.printStackTrace();
            } catch (NotRegisteredException e) {
                e.printStackTrace();
            ...
            if (isUpkeepByPlot()) {
            	if (isTownLevelModifiersAffectingPlotBasedUpkeep())
            		amount = (((getTownUpkeep() * multiplier) * Double.valueOf(getTownLevel(town).get(Settings.TownLevel.UPKEEP_MULTIPLIER).toString())) * nationMultiplier);
            	else
            		amount = (getTownUpkeep() * multiplier) * nationMultiplier;
            	if (Settings.getPlotBasedUpkeepMinimumAmount() > 0.0 && amount < Settings.getPlotBasedUpkeepMinimumAmount())
               		amount = Settings.getPlotBasedUpkeepMinimumAmount();
                return amount;
            } else
            ...
            if (isUpkeepByPlot()) {
            	if (isTownLevelModifiersAffectingPlotBasedUpkeep())
            		amount = (getTownUpkeep() * multiplier) * Double.valueOf(getTownLevel(town).get(Settings.TownLevel.UPKEEP_MULTIPLIER).toString());
            	else
		...
		if (nation != null) {
			if (isNationUpkeepPerTown()) {
				if (isNationLevelModifierAffectingNationUpkeepPerTown())
					return (getNationUpkeep() * nation.getTowns().size()) * Double.valueOf(getNationLevel(nation).get(Settings.NationLevel.UPKEEP_MULTIPLIER).toString());
				else
					return (getNationUpkeep() * nation.getTowns().size());
			} else {
				multiplier = Double.valueOf(getNationLevel(nation).get(Settings.NationLevel.UPKEEP_MULTIPLIER).toString());
			}
		} else
			multiplier = 1.0;
	...
	public static long getBackupLifeLength() {
		...
		long t = TimeTools.getMillis(Settings.getString(ConfigNodes.PLUGIN_BACKUPS_ARE_DELETED_AFTER));
