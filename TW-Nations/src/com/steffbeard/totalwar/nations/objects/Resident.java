...
public class Resident extends NationsObject implements NationsInviteReceiver, EconomyHandler, TownBlockOwner {
	...
	public void setJailed(boolean isJailed) {
		...
		if (isJailed)
			NationsUniverse.getInstance().getJailedResidentMap().add(this);
		else 
			NationsUniverse.getInstance().getJailedResidentMap().remove(this);
	}
	...
	public void sendToJail(Player player, Integer index, Town town) {
		...
		Messages.sendMsg(player, Settings.getLangString("msg_you_have_been_sent_to_jail"));
		Messages.sendPrefixedTownMessage(town, String.format(Settings.getLangString("msg_player_has_been_sent_to_jail_number"), player.getName(), index));
	...
	public void freeFromJail(Player player, Integer index, boolean escaped) {
		...
		this.setJailTown(" ");
		if (!escaped) {
			Messages.sendMsg(this, Settings.getLangString("msg_you_have_been_freed_from_jail"));
			Messages.sendPrefixedTownMessage(town, String.format(Settings.getLangString("msg_player_has_been_freed_from_jail_number"), this.getName(), index));
		} else
			try {
				Messages.sendGlobalMessage(String.format(Settings.getLangString("msg_player_escaped_jail_into_wilderness"), player.getName(), NationsUniverse.getInstance().getDataSource().getWorld(player.getLocation().getWorld().getName()).getUnclaimedZoneName()));
		...
		if (this.isJailed) {
			...
			try {
				...
				if (days > 0) {
					...
					Messages.sendMsg(player, String.format(Settings.getLangString("msg_you've_been_jailed_for_x_days"), days));
				}
			} catch (NationsException e) {
				e.printStackTrace();
			}
		}
		NationsUniverse.getInstance().getDataSource().saveResident(this);
	}
	...
	public Town getTown() throws NotRegisteredException {
		...
		if (hasTown())
			...
			throw new NotRegisteredException(Settings.getLangString("msg_err_resident_doesnt_belong_to_any_town"));
	}
...
//	}	
	...
	public void toggleMode(String[] newModes, boolean notify) {
		...
		if (notify)
			Messages.sendMsg(this, (Settings.getLangString("msg_modes_set") + StringMgmt.join(getModes(), ",")));
	}
	...
	public void clearModes() {
		...
		if (BukkitTools.scheduleSyncDelayedTask(new SetDefaultModes(this.getName(), true), 1) == -1)
			Messages.sendErrorMsg(Settings.getLangString("msg_err_could_not_set_default_modes_for") + getName() + ".");
	...
	public void newReceivedInvite(Invite invite) throws TooManyInvitesException {
		if (receivedinvites.size() <= (InviteHandler.getReceivedInvitesMaxAmount(this) -1)) { // We only want 10 Invites, for residents, later we can make this number configurable
			...
			throw new TooManyInvitesException(String.format(Settings.getLangString("msg_err_player_has_too_many_invites"),this.getName()));
		}
	}
	...
	public String getFormattedName() {
		if (isKing()) {
			return (hasTitle() ? getTitle() + " " : Settings.getKingPrefix(this)) + getName() + (hasSurname() ? " " + getSurname() : Settings.getKingPostfix(this));
		}
			
		if (isMayor()) {
			return (hasTitle() ? getTitle() + " " : Settings.getMayorPrefix(this)) + getName() + (hasSurname() ? " " + getSurname() : Settings.getMayorPostfix(this));
		}
	...
	public String getNamePrefix() {
		if (isKing())
			return Settings.getKingPrefix(this);
		if (isMayor())
			return Settings.getMayorPrefix(this);
		return "";
	}
	...
	public String getNamePostfix() {
		if (isKing())
			return Settings.getKingPostfix(this);
		if (isMayor())
			return Settings.getMayorPostfix(this);
		return "";
	}
