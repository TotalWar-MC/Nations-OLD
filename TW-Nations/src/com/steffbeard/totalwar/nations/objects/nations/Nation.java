...
public class Nation extends NationsObject implements ResidentList, NationsInviter, Bank {

	private static final String ECONOMY_ACCOUNT_PREFIX = Settings.getNationAccountPrefix();
	...
	private boolean isPublic = Settings.getNationDefaultPublic();
	private boolean isOpen = Settings.getNationDefaultOpen();
	...
	public void setCapital(Town capital) {
		...
		try {
			recheckTownDistance();
			NationsPerms.assignPermissions(capital.getMayor(), null);
		} catch (Exception e) {
	...
	public Location getNationSpawn() throws NationsException {
		if(nationSpawn == null){
			throw new NationsException(Settings.getLangString("msg_err_nation_has_not_set_a_spawn_location"));
		}
	...
	public void setNationSpawn(Location spawn) throws NationsException {
		Coord spawnBlock = Coord.parseCoord(spawn);
		TownBlock townBlock;
		NationsWorld world = NationsUniverse.getInstance().getDataSource().getWorld(spawn.getWorld().getName()); 
		if (world.hasTownBlock(spawnBlock))
			townBlock = world.getTownBlock(spawnBlock);
		else 
			throw new NationsException(String.format(Settings.getLangString("msg_cache_block_error_wild"), "set spawn"));

		if(Settings.getBoolean(ConfigNodes.GNATION_SETTINGS_CAPITAL_SPAWN)){
			if(this.capital == null){
				throw new NationsException(Settings.getLangString("msg_err_spawn_not_within_capital"));
			}
			...
			if(!townBlock.hasTown()){
				throw new NationsException(Settings.getLangString("msg_err_spawn_not_within_nationtowns"));
			}
	...
	private void remove(Town town) {
		...
		for (Resident res : titleRemove) {
			...
			NationsUniverse.getInstance().getDataSource().saveResident(res);
		}
	 ...
	 */
	public void recheckTownDistance() throws NationsException {
		if(capital != null) {
	...
	public void setKing(Resident king) throws NationsException {

		if (!hasResident(king))
			throw new NationsException(Settings.getLangString("msg_err_king_not_in_nation"));
		if (!king.isMayor())
			throw new NationsException(Settings.getLangString("msg_err_new_king_notmayor"));
	...
	public void collect(double amount) throws EconomyException {
		...
		if (Settings.isUsingEconomy()) {
			double bankcap = Settings.getNationBankCap();
			if (bankcap > 0) {
				if (amount + this.getAccount().getHoldingBalance() > bankcap) {
					Messages.sendPrefixedNationMessage(this, String.format(Settings.getLangString("msg_err_deposit_capped"), bankcap));
					return;
				}
			}
	...
	public void withdrawFromBank(Resident resident, int amount) throws EconomyException, NationsException {
		...
		//	throw new NationsException(Settings.getLangString("msg_no_access_nation_bank"));
	...
	public void newReceivedInvite(Invite invite) throws TooManyInvitesException {
		if (receivedinvites.size() <= (InviteHandler.getReceivedInvitesMaxAmount(this) -1)) {
			...
			throw new TooManyInvitesException(String.format(Settings.getLangString("msg_err_nation_has_too_many_requests"),this.getName()));
		}
	}
	...
	public void newSentInvite(Invite invite) throws TooManyInvitesException {
		if (sentinvites.size() <= (InviteHandler.getSentInvitesMaxAmount(this) -1)) {
			...
			throw new TooManyInvitesException(Settings.getLangString("msg_err_nation_sent_too_many_invites"));
		}
	}
	...
	public void newSentAllyInvite(Invite invite) throws TooManyInvitesException {
		if (sentallyinvites.size() <= InviteHandler.getSentAllyRequestsMaxAmount(this) -1) {
			...
			throw new TooManyInvitesException(Settings.getLangString("msg_err_nation_sent_too_many_requests"));
		}
	}
	...
	public String getFormattedName() {
		return Settings.getNationPrefix(this) + this.getName().replaceAll("_", " ")
			+ Settings.getNationPostfix(this);
	}

	public void addMetaData(CustomDataField md) {
		super.addMetaData(md);

		NationsUniverse.getInstance().getDataSource().saveNation(this);
	}
